%% Initialize Polya Urns (Proportional Method)
...
% related to pixel i.
function urns = initialize_polya_urns_proportional(image)
...
%    end
    ...
    % Create maximum value for black balls
    max_black = 255;
    ...
    urnCols = double(imageCols);
    % Get the number of elements
    %numCols = length(imageCols);
    % Now get the row elements
    %rows = 1:numCols;
    % Create the super urn. Black is first column. White is second column.
    urns = [max_black - urnCols, urnCols];
